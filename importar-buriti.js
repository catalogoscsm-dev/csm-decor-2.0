...
/**
 * importar-buriti.js
 * Busca produtos do Estofados Buriti via WooCommerce Store API
 * e os injeta no produtos.html do CSM Decor.
 *
 * Uso: node importar-buriti.js
 * Pré-requisito: rode baixar-imagens-buriti.js antes
 */
...
const FORNECEDOR_API  = 'https://estofadosburiti.com.br/wp-json/wc/store/v1/products';
const WPP_NR          = '5519990034068';
const PRODUTOS_HTML   = path.join(__dirname, 'produtos.html');
const LOCAL_IMGS_FILE = path.join(__dirname, 'buriti-imagens-locais.json');
...
const TAGLINE_OVERRIDE = {
  'sonata': 'Formas arredondadas e linhas suaves · Espuma D33 + Pillow Top 18 cm · Base madeira efeito flutuante',
};
const DESCRIPTION_OVERRIDE = {
  'sonata': 'O Sofá Sonata combina formas arredondadas com linhas suaves e pés de madeira embutidos em uma proposta de design contemporâneo e elegante. Assento fixo com espuma D-33 soft e Pillow Top de 18 cm garante conforto excepcional. Base em madeira maciça com efeito flutuante. Suporta até 130 kg por assento.',
};

const CAT_TO_TIPO = {
  'sofa': 'sofa', 'sofas': 'sofa', 'estofados': 'sofa',
  'poltronas': 'poltrona', 'poltrona': 'poltrona',
  'chaise': 'sofa', 'modular': 'sofa',
...
function buildCard(p) {
  ...
  return `
          <article class="cat-card" data-tipo="${tipo}" data-id="buriti-${p.id}" data-fornecedor="buriti"
...
function injectIntoHtml(cards) {
  ...
  content = content.replace(
    /\n\s*<!-- INICIO BURITI -->[\s\S]*?<!-- FIM BURITI -->/g, ''
  );
...
async function main() {
  console.log('\n🔍 Buscando produtos do Buriti Estofados...\n');
  ...
  console.log(`\n🎉 produtos.html atualizado com ${selected.length} produtos do Buriti!\n`);
}
