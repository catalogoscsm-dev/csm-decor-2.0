#!/usr/bin/env node
/**
 * baixar-imagens-buriti.js
 * Baixa imagens dos produtos Estofados Buriti via WooCommerce Store API,
 * normaliza para 600x600 e salva em imagens/buriti/
 *
 * Uso: node baixar-imagens-buriti.js
 */
...
const OUT_DIR        = path.join(__dirname, 'imagens', 'buriti');
const SIZE           = 600;
...
async function main() {
  ...
  for (const p of selected) {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      if (fs.existsSync(outPath)) {
        ...
        localImgs.push(`imagens/buriti/${fname}`);
        continue;
      }
  ...
  fs.writeFileSync(
    path.join(__dirname, 'buriti-imagens-locais.json'),
    JSON.stringify(localMap, null, 2), 'utf8'
  );

  console.log(`📦 Imagens salvas em: imagens/buriti/`);
  console.log(`📄 Mapa salvo em:     buriti-imagens-locais.json`);
  console.log(`\nAgora rode: node importar-buriti.js\n`);
}
